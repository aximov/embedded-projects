...
### センサーの方向によって重力の出方が変わってしまう問題

ここまで、z軸は重力がうまく測れず、-1 が出てしまうという問題が継続して起きている。変換前の raw の値も -1 で出ているし、x, y 軸の値はほぼ正常に出ているし、z 軸の値もたまに900前後の値が出ることから、配線などの物理的な原因があるのではないか。

そこで一旦配線をバラし、センサーもブレッドボードから一度外し、ブレッドボード上の位置も変え、ジャンパ線を全て取り替えて配線を組み直した。GNDとVINの短絡がないことを確認後、再び測定をした。

しかしやはり az は -1 で出てしまい、また ax, ay についてもある向きでは900程度だが上下を逆にすると -200 前後または 0 になるという症状が出るようになった。

原因切り分けのために、次はブレッドボードを経由せず、BNO055 を直接 Nucleo に接続してみる。